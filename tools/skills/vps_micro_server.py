...
def get_rag_report():
    import subprocess
    # Először frissítjük a fájlt a legújabb adatokkal
    try:
        subprocess.run(["python3", "/home/misi/Jules_mx/scripts/generate_markdown_report.py"], check=True)
    except Exception as e:
        pass

    report_path = "/home/misi/Jules_mx/alerts/alerts_summary.md"
    if os.path.exists(report_path):
        with open(report_path, "r", encoding="utf-8") as f:
            return {"status": "success", "report": f.read()}
    return {"status": "error", "message": "Report not found or not generated yet."}
