...
# Itt fogjuk legenerálni az optimális JSONL fájlt a Gerilla RAG-hoz.
OUTPUT_FILE = "Gerilla_RAG_data.jsonl"
OUTPUT_ZIP = "Gerilla_RAG_knowledge.zip"
OUTPUT_LIST = "processed_repos_gerilla.txt"

# Kód, konfiguráció és dokumentáció kiterjesztések
# FIGYELEM: A média fájlokat (jpg, png, mp4, wav, stb.) SZIGORÚAN KIZÁRJUK!
VALID_EXTENSIONS = {
    # Python ökoszisztéma (Agentek és Skriptek)
    '.py', '.pyx', '.pxd', '.ipynb',
    # C/C++ / Assembly (Evasion, Syscallok, Injectorok, Black Ops)
    '.c', '.cpp', '.h', '.hpp', '.cc', '.cxx', '.asm', '.s',
    # Config és strukturált adatok
    '.json', '.yaml', '.yml', '.toml', '.ini', '.cfg',
    # Dokumentáció és leírások
    '.md', '.rst', '.txt',
    # Web / Frontend Interfész (Proxy, Stealth)
    '.js', '.ts', '.jsx', '.tsx', '.vue', '.svelte', '.html', '.css', '.scss',
    # Shell / Scripting (Automatizálás, Reverse shell)
...
def get_programming_language(ext):
    ...
    mapping = {
        ...
        '.c': 'C', '.cpp': 'C++', '.h': 'C/C++ Header', '.asm': 'Assembly', '.s': 'Assembly',
        '.js': 'JavaScript', '.ts': 'TypeScript', '.jsx': 'React JSX', '.tsx': 'React TSX',
        '.vue': 'Vue', '.svelte': 'Svelte', '.html': 'HTML', '.css': 'CSS', '.scss': 'SCSS',
        '.json': 'JSON', '.yaml': 'YAML', '.yml': 'YAML', '.toml': 'TOML',
        '.md': 'Markdown', '.txt': 'Text',
        '.sh': 'Shell', '.bash': 'Shell', '.bat': 'Batch', '.cmd': 'Batch', '.ps1': 'PowerShell', '.vbs': 'VBScript'
    }
...
def main():
    print("=== 🧠 video_picture_restoration - STRUKTURÁLT KNOWLEDGE BUILDER ===")
